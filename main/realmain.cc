...
#include "core/GlobalSubstitution.h" // TODO(jez) don't commit this
...
int realmain(int argc, char *argv[]) {
    ...
    if (opts.runLSP) {
        ...
        if (!opts.minimizeRBI.empty()) {
            ...
            {
                // TODO(jez) This is turning out to be a huge pain. Maybe punt on getting the
                // NameRef's to be comparable, and just use strings?
                // Another option: do the pipeline::index for the minimizeRBI at the same time as
                // the main pipeline index step, so that there will be an even number of symbols at
                // the time GlobalSubstitution runs.

                // Puts any names exclusive to gsForMinimize into gs
                auto optionalCommonParent = nullptr;
                auto skipFiles = true;
                auto substitution = core::GlobalSubstitution{*gsForMinimize, *gs, optionalCommonParent, skipFiles};

                for (auto &tree : indexed) {
                    auto file = tree.file;
                    if (!file.data(*gsForMinimize).cached) {
                        auto ctx = core::MutableContext{*gs, core::Symbols::root(), file};
