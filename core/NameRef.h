...
public:
    ...
    constexpr NameRef(WellKnown, NameKind kind, u4 id) : _id{(id << KIND_BITS) | static_cast<u4>(kind)} {
    ...
    static NameRef fromRaw(const GlobalState &gs, u4 raw) {
        return NameRef(gs, NameRef::fromRawUnchecked(raw));
    }

    static NameRef fromRawUnchecked(u4 raw) {
        auto ref = NameRef();
        ref._id = raw;
        return ref;
    }
    ...
    u4 uniqueIndex() const {
        ENFORCE_NO_TIMER(kind() == NameKind::UNIQUE);
        return unsafeTableIndex();
    }

    u4 constantIndex() const {
